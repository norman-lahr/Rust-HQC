...
use crate::parameters::{HQC_1, SHARED_SECRET_BYTES};
...
pub fn crypto_kem_keypair_ref(p: &crate::parameters::HqcParameters, ) -> (Vec<u8>, Vec<u8>) {
    let mut ek_kem = vec![0u8; p.ek_bytes];
    let mut dk_kem = vec![0u8; p.dk_bytes];
    unsafe {
...
/// same entropy/personalization used to seed the Rust `prng_reader`)
/// before invoking this wrapper for a meaningful cross-validation.
pub fn crypto_kem_enc_ref(p: &crate::parameters::HqcParameters, ek_kem: &[u8]) -> (Vec<u8>, Vec<u8>) {
    let mut c_kem = vec![0u8; p.ct_bytes];
...
fn test_kem_keygen() {
    ...
    for i in 0..TEST_ROUNDS {
        ...
        let (ek_rs, dk_rs) = crate::kem::crypto_kem_keypair(p, &mut prng_reader);
        let (ek_ref, dk_ref) = crypto_kem_keypair_ref(p, );
...
fn test_kem_enc() {
    ...
    for i in 0..TEST_ROUNDS {
        ...
        let (ek_kem, _dk_kem) = crate::kem::crypto_kem_keypair(p, &mut kg_reader);

        prng_init_ref(&entropy, &pers);
        let (ek_kem_ref, _dk_kem_ref) = crypto_kem_keypair_ref(p, );
        ...
        let (c_kem_rs, k_rs) = crate::kem::crypto_kem_enc(p, &mut enc_reader, &ek_kem);
        let (c_kem_ref, k_ref) = crypto_kem_enc_ref(p, &ek_kem);
...
fn test_kem_dec() {
    ...
    for i in 0..TEST_ROUNDS {
        ...
        let mut kg_reader = crate::symmetric::prng_init(&entropy, &pers);
        prng_init_ref(&entropy, &pers);
        let (ek_kem, dk_kem) = crate::kem::crypto_kem_keypair(p, &mut kg_reader);
        let (ek_kem_ref, dk_kem_ref) = {
            prng_init_ref(&entropy, &pers);
            crypto_kem_keypair_ref(p, )
        };
        ...
        let mut enc_reader = crate::symmetric::prng_init(&entropy, &pers);
        prng_init_ref(&entropy, &pers);
        let (c_kem, _k) = crate::kem::crypto_kem_enc(p, &mut enc_reader, &ek_kem);
        let (c_kem_ref, _k_ref) = {
            prng_init_ref(&entropy, &pers);
            crypto_kem_enc_ref(p, &ek_kem)
        };
        assert_eq!(c_kem, c_kem_ref);

        // The actual test: dec has no PRNG dependency
        let k_prime_rs = crate::kem::crypto_kem_dec(p, &c_kem, &dk_kem);
...
fn test_kem_roundtrip() {
    ...
    for i in 0..TEST_ROUNDS {
        ...
        let mut enc_reader = crate::symmetric::prng_init(&entropy, &pers);
        let (c_kem, k) = crate::kem::crypto_kem_enc(p, &mut enc_reader, &ek_kem);

        let k_prime = crate::kem::crypto_kem_dec(p, &c_kem, &dk_kem);
